...
class Connect
{
    ...
    public function insert($data)
    {
        ...
        $pass = preg_match('/^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[!@#$%^&*]).{8,}+$/', $password);
        ...
        if ($password != $cpassword) {
            $hasErrors = true;
            echo "Password and confirm password not same <br>";
        }

        $encpass = md5($password);

        if (!$hasErrors) {
            $sql = "INSERT INTO users(name,password) Values ('$name','$encpass')";
            if ($this->conn->query($sql)) {
                echo "<script>alert('Signed Up!!');
    ...
    public function check($data)
    {
        ...
        if (!$hasErrors) {
            ...
            $sql = "SELECT * FROM users WHERE name='$name' AND password='$encpass'";
